        ...
        if (Network.isAvailable(mContext)) {
            try {
                ...
                if (mStore == Store.GOOGLE_PLAY) {
                    HttpGet request = new HttpGet(UpdateChecker.GITHUB_VERSIONS_FILE); // load the versions file
                    ...
                    JSONObject versions = new JSONObject(responseStringBuilder.toString());
                    mVersionDownloadable = versions.getString("versionName");
